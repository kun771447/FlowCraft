  ...
  function convertStoredEventsToSteps(events: StoredEvent[]): Step[] {
    ...
    for (const event of events) {
      switch (event.messageType) {
        ...
        case "CUSTOM_SCROLL_EVENT": {
          const scrollEvent = event as StoredCustomScrollEvent;
          if (
            scrollEvent.url &&
            (scrollEvent.scrollX !== undefined ||
              scrollEvent.scrollY !== undefined)
          ) {
            const step: ScrollStep = {
              type: "scroll",
              timestamp: scrollEvent.timestamp,
              tabId: scrollEvent.tabId,
              scrollX: scrollEvent.scrollX,
              scrollY: scrollEvent.scrollY,
              url: scrollEvent.url,
              frameUrl: scrollEvent.frameUrl,
              xpath: scrollEvent.xpath,
              cssSelector: scrollEvent.cssSelector,
              elementTag: scrollEvent.elementTag,
              screenshot: scrollEvent.screenshot,
            };
            steps.push(step);
          } else {
            console.warn(
              "Skipping incomplete CUSTOM_SCROLL_EVENT:",
              scrollEvent
            );
          }
          break;
